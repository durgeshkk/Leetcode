...
int recur(ll idx,vector<int> &v,int l){
    if(idx >= v.size()){
        if(l == k){
            return 0;
        }else{
    ...
    return dp[{idx,l}] = min(a,b);
}

int minCapability(vector<int>& v, int a) {
    k = a;
    ll l = 0,h = 1e9;
    while(l <= h){
        ll mid = (l+h)/2;
