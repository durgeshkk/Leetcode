...
int maximumBags(vector<int>& c, vector<int>& v, int k) {
    int ans = 0;
    int n = v.size();
    vector<int> tmp;
    for(ll i = 0;i<n;++i){
...
int minimumLines(vector<vector<int>>& v) {
    sort(v.begin(),v.end());
    ll ans = 0;
    ll mx = -1,my = 0,n = v.size();
    for(ll i = 1;i<n;++i){
        if(i == 1){
            mx = v[i][0]-v[i-1][0];
            my = v[i][1]-v[i-1][1];
            ll g = __gcd(abs(mx),abs(my));
            mx /= g;my /= g;ans = 1;
        }else{
            ll smx = v[i][0]-v[i-1][0];
            ll smy = v[i][1]-v[i-1][1];
            ll g = __gcd(abs(smx),abs(smy));
            smx /= g;smy /= g;
            if(smx == mx and smy == my){}
            else{
                ++ans;
                mx = smx,my = smy;
            }
...
int totalStrength(vector<int>& st) {
    ...
    long long ans = 0;
    for (int i = 0; i < N; ++i) {
        ans += ((prefix_sum[right[i] + 1] - prefix_sum[i + 1]) * (i - left[i]) % mod + mod * 2 - 
