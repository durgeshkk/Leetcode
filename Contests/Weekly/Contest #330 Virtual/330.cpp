...
// 1st & last elements will always be in the ans
// Now we just need to add 2 such neighbour elements that will result in max sum
...
long long putMarbles(vector<int>& wt, int k) {
    ...
    ll ans = (wt[0]+wt[n-1]);
    ll ans2 = 0,ans1 = 0;
    // ans2 will say pick max (k-1) elements from v
    for(ll i = 0;i<k-1;++i){
        ans2+=v[v.size()-1-i];
        ans1 += v[i];
    }
